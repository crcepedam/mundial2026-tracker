...
function predictScoreDixonColes(home, away, probHome, probAway) {
  ...
  if (result === 'H') {
    // Victoria local — usar lambdas para variedad de marcadores
    // WC2026 victorias locales reales: 2-0, 4-1, 2-1, 7-1, 5-1, 3-1, 3-0, 4-2, 1-0, 1-0
    if (pH > 0.85)      { hg = lH >= 2.0 ? 3 : 2; ag = 0; }                     // Aplastante → 3-0 o 2-0
    else if (pH > 0.70) { hg = 3; ag = lA >= 0.9 ? 1 : 0; }                     // Claro → 3-1 o 3-0
    else if (pH > 0.60) { hg = 2; ag = lA >= 0.9 ? 1 : 0; }                     // Moderado → 2-1 o 2-0
    else                { hg = lH >= 1.5 ? 2 : 1; ag = lA >= 0.9 ? 1 : 0; }     // Leve → 2-1, 2-0, 1-0
  }
  else if (result === 'A') {
    // Victoria visitante — WC2026: 0-1, 1-4, 1-3, 1-2
    if (pA > 0.85)      { hg = 0; ag = lA >= 2.0 ? 3 : 2; }
    else if (pA > 0.70) { hg = lH >= 0.9 ? 1 : 0; ag = 3; }
    else if (pA > 0.60) { hg = lH >= 0.9 ? 1 : 0; ag = 2; }
    else                { hg = lH >= 1.2 ? 1 : 0; ag = lA >= 1.5 ? 2 : 1; }
  }
  else {
    // Empate — WC2026 J1: 1-1(7x), 0-0(1x), 2-2(2x)
    const avgL = (lH + lA) / 2;
    if (Math.max(lH, lA) < 0.7)  { hg = 0; ag = 0; }   // Ambos débiles → 0-0
    else if (avgL >= 1.5)         { hg = 2; ag = 2; }   // Ambos fuertes → 2-2
